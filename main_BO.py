...
=======
    def Objective_function(self, BSpline, verbose = False):
        '''Exists just to simplify and discretize away this step in a nice way.'''
        self.trajectory_offsets = np.abs(interpolate.splev(np.linspace(0,1,200, endpoint = False),BSpline))
        self.trajectory_nodes = self.offset(sef.Tip_points, self.trajectory_offsets)
...
=======
        ...
        while True:
            if abs(self.offset-self.prev_offset)<0.002:
                break
            print(self.offset)
